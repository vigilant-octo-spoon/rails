class FollowsMethodologyController < ApplicationController
  ...
  def all_users
    ...
    @user = User.where(authentication_token: auth_token, admin: true).first
    if @user
      @users = User.where(admin: false)
      json_user = []
      @users.each do |us|
        folow_met = FollowsMethodology.where(user_id: us.id)
        follows = []
        folow_met.each do |fll|
          #### INFO F #####
          methodology_name = Methodology.find_by_id(fll.methodology_id).title
        ...
        user_ready = {:name => us.name, :last_name =>us.last_name, :email => us.email, :follows => follows}
        json_user.push(user_ready)
      end
      json_final = { :users => json_user}.to_json
      render json: json_final, status: :ok
    else
      render json: {"message": "Esto es información privada, no puedes acceder a ella"}, status: :fail
    end
  end
